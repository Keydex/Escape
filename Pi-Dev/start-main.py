...
#        trigger_END = GPIO.input(2)
        ...
        if (player.position() > winEnd_time and player.position() < (winEnd_time + 4)):
             print('Video (WIN) Ended, Resetting Back to Intro')
             resetCheck = True
        if ((currentTime - startTime) > 3000 and endCheck == False and startCheck == True):#If 50 minutes pass jump to ten minute segment
